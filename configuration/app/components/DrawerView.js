		...
		return (
			<View style={ [ styles.container, this.props.sceneStyle ] }>
				...
				<List containerStyle={ styles.list }>
					...
					{ ( !isAnonymous ) &&
					<ListItem onPress={ () => this.openRoute( '/paymentmethod' ) } title="Payment Methods"
							  hideChevron={ true }/> }
					{ ( !isAnonymous ) &&
					<ListItem onPress={ () => this.openRoute( '/user/location' ) } title="Set Location"
							  hideChevron={ true }/> }
					{ ( !isAnonymous ) && <ListItem onPress={ () => this.openRoute( '/user/profile' ) } title="Profile"
													hideChevron={ true }/> }
					{ ( !isAnonymous ) &&
					<ListItem onPress={ () => this.openRoute( '/user/messages/thread/55' ) } title="Test Thread"
							  hideChevron={ true }/> }
					{ ( !isAnonymous ) && <ListItem onPress={ () => this.openRoute( '/user/logout' ) } title="Log Out"
													hideChevron={ true }/> }
				</List>
			</View>
		)
